...
class OrderViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        AnalyticsManager.sharedInstance?.analytics?.logPage("Order asc")
        
		
        items = [
			MenuItem(text: "buildup",
                action: NavigationAction<BuildupMenuItem3ViewController>(rootViewController: self),
				image: "DefaultMenuIcon.png",
				textStyle: TextStyle(alignment: .Left)
            ),
			MenuItem(text: "Local",
                action: NavigationAction<LocalMenuItem4ViewController>(rootViewController: self),
				image: "DefaultMenuIcon.png",
				textStyle: TextStyle(alignment: .Left)
            ),
			MenuItem(text: "Excel",
                action: NavigationAction<ExcelMenuItem1ViewController>(rootViewController: self),
				image: "DefaultMenuIcon.png",
				textStyle: TextStyle(alignment: .Left)
            ),
			MenuItem(text: "googlesheet",
                action: NavigationAction<GooglesheetMenuItem1ViewController>(rootViewController: self),
				image: "DefaultMenuIcon.png",
				textStyle: TextStyle(alignment: .Left)
            ),
			MenuItem(text: "Googledrive",
                action: NavigationAction<GoogledriveMenuItem2ViewController>(rootViewController: self),
				image: "DefaultMenuIcon.png",
				textStyle: TextStyle(alignment: .Left)
            ),
